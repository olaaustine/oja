...
class BodyPartExerciseForm(forms.ModelForm):
    ...
    exercise_description = forms.CharField(
        widget=forms.Textarea, label="Exercise Description", required=False
    )
    ...
    def clean(self):
        ...
        if existing_exercise:
            self.add_error(
                "exercise_name", f"{exercise_name} already exists for {body_part}."
            )
...
class WorkoutSessionForm(forms.ModelForm):
    ...
    exercise_name = forms.CharField(max_length=100, label="Exercise Name")
    exercise_description = forms.CharField(
        widget=forms.Textarea, label="Exercise Description"
    )
